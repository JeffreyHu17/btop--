...
bool DaemonManager::daemonize() {
    std::cerr << "daemon mode is not supported on Windows; use install.ps1 to register btop-agent as a scheduled task" << std::endl;
    return false;
}
...
bool DaemonManager::shutdownRequested() const {
    return pImpl_->shutdown_requested_;
}

void DaemonManager::gracefulShutdown() {
    pImpl_->shutdown_requested_ = true;
}
